...
class MakeRoomAdminRestServlet(ResolveRoomIdMixin, RestServlet):
    ...
    async def on_POST(
        ...
        new_pl_content["users"][user_to_add] = 100
        ...
        persisted_context = await context.persist(event)
        await self.event_creation_handler.persist_and_notify_client_events(
            fake_requester, [(event, persisted_context)], ratelimit=False, extra_users=[]
        )
